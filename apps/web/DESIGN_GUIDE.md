...
6. **악센트(에메랄드 `--accent: #0f7256`)는 절제.** 1차 버튼·링크·포커스 링·active nav에만. 랜딩(install-page)과 동일한 크림 페이퍼 + 에메랄드 팔레트로 통일(인디고 폐기). **장식용 보라 그라데이션 금지** (`.sidebar__logo`, `.progress__bar`의 `135deg accent→#7c3aed`는 평면 `var(--accent)`로). 코드에 raw hex 금지 — 색은 CSS 변수로만.
