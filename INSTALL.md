...
이 문서는 **AI 에이전트(Claude Code 또는 Codex)가** 사용자의 컴퓨터에 CareerMate를 설치·초기화할 때 한 단계씩 따라가는 런북입니다. 사람용 튜토리얼이 아니라, **당신(AI)이 실행하고 사용자에게 한국어로 설명하는** 절차서입니다.
...
1. **동의 먼저.** 파일을 만들거나 서버를 켜기 **전에** 무엇을 할지 설명하고 사용자 동의를 받습니다. (4단계 체크리스트 참고)
2. **설명 먼저, 실행 나중.** 각 명령을 실행하기 전에 "지금 무엇을, 왜 하는지" 한 줄로 알립니다.
...
## 4단계 — 사용자 동의 항목 (여기서 멈추고 동의를 받으세요)

아래 항목을 사용자에게 그대로 보여주고 **동의를 받은 뒤에만** 다음 단계로 진행합니다.

설치를 진행하면 CareerMate는 다음을 수행합니다. 동의하시나요?

- [ ] 데이터 폴더 **`~/.careermate`** 생성
- [ ] 로컬 **SQLite 데이터베이스**(`careermate.sqlite`) 생성
- [ ] 로컬 **대시보드 서버**(`http://127.0.0.1:4319`) 실행 — 외부 접근 불가
...
- Claude Code라면 **1회 승인** 프롬프트를 수락합니다.
...
연결 검증까지 끝났으면, 사용자에게 아래 "완료 카드"를 쉬운 한국어로 보여주고 마무리하세요(도구 이름은 노출하지 말 것). **지금 실행 중인 앱에 맞는 항목만 골라** 안내합니다(공통 '붙여넣기'는 항상 포함).
