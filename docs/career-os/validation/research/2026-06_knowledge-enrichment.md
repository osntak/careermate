...
- **적용·검증·머지 완료(라이브):** `knowledge/anchors/` 자산 8종 + README **+ serve-wiring**(get_playbook 자동첨부·`get_anchor` 도구·라우트 안내·권한 4곳 동기화). typecheck·86/86 테스트·기능검증 8/8·**K=5 A/B 무회귀**(§7)·빌드 green → **main `0ca193a`로 머지**(브랜치 rebase+ff, docx 작업과 파일 비중복으로 충돌 0).
...
- **보류(출처검증 후):** career-description [2차] — Wanted +13%/36% 정량화 통계는 **원티드 공식 블로그/CV로 출처 추적됨**([2차] 벤더)이나 anchor 부적합(룩업 아닌 동기부여 통계)이라 자산화 안 함. 직무별 키워드·Searchright 3-템플릿은 데이터 미검증 보류.
- **원천 데이터:** 본 라운드 워크플로우 산출물(synth 3종·coverage 4종·findings 19종)은 세션 스크래치에 보존. 정본 결론은 본 문서.

## 7. K=5 A/B 회귀 검증 (PROTOCOL §10)

앵커 배선(`get_playbook` 자동첨부)이 산출물 품질을 회귀시키는지 K=5 A/B로 검증했다(A=앵커 없음/기존, B=앵커 첨부/신규, 블라인드 채점). 5 시나리오 × 2조건 × K=5 = 50생성·50채점.

| 시나리오 | A median | B median | 델타(B−A) | 판정 | A/B modal | crit 재현(A/B) |
|---|---:|---:|---:|---|---|---|
| fit-s1 | 91 | 88 | −3 | 무회귀 | pass/pass | 0/0 |
| fit-s2 | 92 | 92 | 0 | 무회귀 | pass/pass | 0/0 |
| cl-s1 | 93 | 91 | −2 | 무회귀 | pass/pass | 0/0 |
| cd-s1 | 92 | 91 | −1 | 무회귀 | pass/pass | 0/0 |
| interview-s1 | 92 | 93 | +1 | 무회귀 | pass/pass | 0/0 |

- **5/5 무회귀**: 델타 −3~+1(평균 −1.0), 전부 ±9(≈2σ/√K) 노이즈 내 · modal verdict 악화 0 · critical 재현결함 증가 0.
- **정직한 한계(상방 미측정):** 이 fixture셋은 A·B 모두 critical 재현결함이 0인 "건강한" 케이스라, 앵커가 노리는 실패 모드(시니어리티/녹아웃/수치 발명)를 애초에 유발하지 않는다. 줄일 결함이 없었으니 §10의 개선 신호(critical 재현결함 감소)는 입증·반증 불가 — **"회귀 없음"만 입증**됐고 효용(상방)은 이 셋에서 보이지 않는다.
- **노이즈는 생성 분산 지배:** median 안정(최대 이동 −3), 단발 딥(B cd-s1=73·B fit-s1=80)은 K=5 표본 흔들림이고 median이 흡수(등급 pass 유지) → 판정기 하향이 아니라 생성 변동성.
- **후속(상방 측정용, 비차단):** 결함-시드 시나리오(시니어리티 불일치·녹아웃 요건 누락·수치 날조 유발)로 A에서 critical 결함이 뜨고 B에서 줄어드는지 봐야 "앵커가 발명을 줄인다"가 §10 기준으로 입증된다. 현 건강 케이스셋으론 영원히 중립.
- **결론: 회귀 0 → 배선 유지 안전, 머지 완료.** (검증 산출물: 101 에이전트·~5.5M 토큰, 세션 스크래치.)
