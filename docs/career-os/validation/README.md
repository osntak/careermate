...
>
> **종료된 내부 R&D 로그 · npm 미출하.** 검증은 R6/R7로 마무리됐다. 결론은 아래 서술형 md로 증류해
> git에 남기고, 기계 생성 원증거(`runs/consensus/**`·`ledger.jsonl`·`r7/research-raw*.json`)와
> 재생성 가능한 하네스 스크래치(`runs/{out,fixtures,...,_probe}`)는 추적하지 않는다(.gitignore).
