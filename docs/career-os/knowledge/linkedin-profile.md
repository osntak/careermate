...
LinkedIn 프로필을 **(1) 리크루터 검색에 노출되고(키워드·완성도), (2) 5~10초 스캔에서 클릭을 유발하며(헤드라인·About 첫 문장), (3) 신뢰를 증명하는(추천서·정량 성과·Featured)** 세 축으로 최적화하기 위한 플레이북과, **LLM 없이 셀 수 있는 자가검증 루브릭**이다. CareerMate에는 LLM이 없으므로, 연결된 외부 AI(Claude/GPT/Gemini)가 이 규칙으로 프로필 초안을 작성·진단한다. 루브릭을 기계적으로(개수·존재·비율·정규식) 실행하는 `validate_linkedin_profile`은 **Phase B 설계이며 현재 미구현** — 지금은 연결 AI가 이 루브릭을 **수동(advisory)**으로 적용한다(§5 Phase B 힌트).
