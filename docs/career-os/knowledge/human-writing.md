...
> **역할 경계:** 이 문서는 글을 **쓰는·생성하는(write)** 단계의 플레이북이다(`get_playbook`, 작성 직전). 다 쓴 초안을 **저장 직전 점검·채점**하는 일은 짝 검증기 [`verifiers/human-voice.md`](verifiers/human-voice.md)(`get_verifier`)가 맡는다 — 같은 주제라도 *쓰기 지침*과 *검증 루브릭*은 역할이 다르고, 두 곳의 강도 차이(예: 사실 보존이 여기선 LLM-플래그, 검증기 C8·C9에선 하드 게이트)는 **의도된 것**이다. 두 문서는 슬롭·과장·클리셰·번역투 사전을 **의도적으로 각자 온전히 복제**해 들고 있다 — 한쪽만 호출돼도 사전이 온전하도록 함이니, **단일 출처로 합치거나 다른 문서 포인터로 대체하지 마라(독립 serve가 깨진다).**
