...
- **C3·C4·C5 위반(단조로움)**: **C3(burstiness)는 권고 신호이므로 재작성을 강제하지 말고** 긴 문장 1~2개를 쪼개 **짧은 강조 문장(≤8단어)** 삽입을 권고만 한다. **C4·C5(게이트)는** 연속으로 같은 시작어를 쓴 문장의 도입부를 바꾸고 3단 병렬을 1개로 줄이거나 문장 형태를 비대칭으로 푸는 재작성을 한다. 표준편차·시작어 분포를 재측정한다.
...
재검증 종료 조건: **C1·C2·C4·C5·C6·C7·C10·C11 전부 합격 + C8·C9 위반 0건**. **C3(문장 길이 다양성: σ≥5 및 최단 문장 ≤8단어 조건 모두)는 게이트가 아니라 권고 신호**다 — 두 하위조건 어느 것이라도 미달해도 단독으로 종료를 막지 않고 개선 권고만 한다(짧은 문항·이력서 bullet 등 통계 무의미 구간 false-positive 방지; §2 advisory note·`human-writing.md` R12와 동일선). 그 외 한 항목이라도 불합격이면 루프를 멈추지 않는다. (후속: C4·C5의 게이트 지위는 human-writing.md 게이트 집합 R5/R6/R9/R11과 잔여 불일치가 있어 후속 정렬 대상[STATUS R7 후속 항목]. 이번 스코프는 R7이 지목한 σ(C3) 결함에 한정.)
