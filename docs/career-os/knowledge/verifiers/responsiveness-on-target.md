...
| C8 | JD 필수 요건 커버리지 | JD의 **필수(required) 요건** 항목 중 산출물(이력서+자소서)에서 다뤄진 비율 **≥ 80%**, 필수 요건 미커버 **0개**. (이 ≥80%는 응답성 커버리지 신호이며 적합(fit) 권장 게이트가 아니다 — 적합 판정은 `fit-matching`/`fit-analysis`가 하드요건(knockout)으로 내린다. C8 자체는 응답성 루프 내 critical 검사로, 미달은 적합·지원을 차단하는 게 아니라 §3 보강 재작성을 트리거한다.) | det/D · JD에서 필수/우대 요건 분리 추출 → 각 요건 키워드가 산출물에 존재하는지 매칭 → (커버 요건 ÷ 전체 필수). 미커버 필수 요건 나열. **축 구분(메타):** 이 C8 = JD *필수요건 응답(response)* 커버리지 ≥80% **critical**. `ats-compat` C8 = JD 키워드 *표면 존재(presence)* M/N≥0.70 **advisory** — **다른 축**이라 두 게이트의 숫자·방향이 달라도 **충돌이 아니다**(요건에 응답해도 ATS 표면 키워드가 모자랄 수 있고, 키워드가 표면에 있어도 요건에 응답 안 할 수 있다) |
...
- **글로벌(JD 중심·LinkedIn):** C8·C9가 응답성 루프 내 1차 게이트(fit 권장 게이트는 아님 — 적합 판정은 fit-matching/fit-analysis가 내린다). JD의 required/preferred 분리와 키워드 커버리지(ATS 정합)가 핵심. 면접은 STAR(C5) 중심. cover letter는 보통 단일 자유 양식이라 C6 대신 **JD 요건 응답성(C8)** 이 주 축.
