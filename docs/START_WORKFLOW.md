...
설치·연결(`INSTALL.md`)이 끝난 직후, AI(당신)가 사용자를 빠르게 생산적으로 만들기 위해 따라가는 런북입니다. 도구는 정해진 순서로 호출하고, 매 단계 무엇을 하고 있는지 사용자의 언어로 쉽게 설명하세요. MCP·job_id 같은 기술 용어는 노출하지 마세요. 확실하지 않은 정보는 지어내지 말고 사용자에게 확인하세요.
...
- 사용자가 올린 이력서/경력기술서/포트폴리오/자기소개서 파일 또는 직접 알려준 내용을 바탕으로 프로필을 구조화합니다.
...
- `get_workflow_guide` — 표준 워크플로우(온보딩, 공고 분석, 자소서 작성, 경력기술서 작성, 상태 관리, 면접 준비, 개인 브랜드)의 단계별 안내. 어떤 순서로 어떤 도구를 호출할지 확인할 때 사용. `workflow_id` 생략 시 전체 목록.
