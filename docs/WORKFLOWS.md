...
- MCP 도구 37종의 입출력 명세는 코드(`packages/mcp-tools/src/tools.ts`)에 정의되어 있으며, AI는 각 도구의 `inputSchema`로 직접 확인할 수 있다.

## 워크플로우 7종 개요 (핵심 6 + build_personal_brand)
...
| `write_career_description` | 경력기술서 작성 | 프로필·이력서·경력·프로젝트·스킬을 근거로 실제 제출 가능한 경력기술서를 작성·저장한다. |
| `manage_application_status` | 지원 상태 관리 | 지원 진행 상황(8단계)을 업데이트하고 다음 행동을 제안한다. |
| `prepare_interview` | 면접 준비 | 공고 기준 예상 질문·STAR 답변·1분 자기소개를 준비·저장한다. |
| `build_personal_brand` | 퍼스널 브랜드 (LinkedIn·포트폴리오) | LinkedIn 프로필·포트폴리오 같은 자기표현 자산을 직무에 맞춰 작성·최적화한다(이력서 임포트와 분리된 선택 작업). |
...
- **쉬운 전달(사용자 언어)**: 분석·결과를 사용자에게 설명할 때는 기술 용어를 빼고 사용자의 언어로 쉽게 전달한다.
...
2. 비어 있는 항목을 사용자의 언어로 쉽게 안내하고, 이력서/경력기술서/포트폴리오/자기소개서 파일 업로드 또는 정보 입력을 요청한다.
...
(공고 원문 읽기)
  ...
  → (사용자의 언어로 쉽게 설명)
  → (자기소개서 작성 여부 선택지 제안)
  → save_cover_letter_version         # 사용자가 "네"를 선택한 경우에만, job_id·note
...
## 4. write_career_description — 경력기술서 작성

프로필·이력서·경력·프로젝트·스킬을 근거로 실제 제출 가능한 경력기술서를 작성하고 문서함에 저장하는 흐름.

**트리거:** 사용자가 "경력기술서 써줘 / 경력기술서 만들어줘 / 내 경력 정리해줘"라고 요청할 때.

**단계:**

1. `get_onboarding_status`로 준비 상태를 확인하고, `get_profile`·`get_resumes`·`get_experiences`·`get_projects`·`get_skills`를 호출해 기존 프로필, 이력서, 경력기술서(`kind=career_description`), 경력, 프로젝트, 기술스택을 모두 확인한다.
2. 사용자가 파일 경로를 주면 `read_document`로 읽고, 인입 폴더를 사용했다면 `read_inbox`로 본문을 읽는다. 이미 저장된 자료와 새 파일을 비교해 중복·상충되는 사실은 표시하고 추측하지 않는다.
3. `get_workflow_guide({ workflow_id: "write_career_description" })`가 안내하는 전문가 절차를 따른다. 작성 직전 `get_playbook({ domain: "resume" })`와 `get_playbook({ domain: "ats" })`를 받아 실제 경력기술서 형식과 ATS 친화성을 적용한다.
4. 작성 전에 목적을 확인한다: 범용 마스터본인지, 특정 직무/공고용인지, 신입/무경력이라 프로젝트·교육·활동 중심 역량기술서로 쓸지, 분량과 톤은 어떻게 할지 묻는다.
5. 경력기술서는 자기소개서처럼 서사로 쓰지 않는다. 제목, 핵심 요약, 핵심 역량, 경력/프로젝트별 역할·기간·도구·문제·행동·성과를 표나 불릿 중심으로 정리한다. 수치·회사명·기간·직책은 저장된 사실만 쓰고, 없는 성과 수치는 만들지 않는다.
6. 무경력/비개발자라면 경력을 날조하지 말고 교육·프로젝트·운영 경험·업무 도구·협업 경험·성과 증거 중심의 "역량기술서"로 구성한다.
7. 저장 직전 `get_verifier({ id: "truthfulness" })`, `get_verifier({ id: "consistency" })`, `get_verifier({ id: "ats-compat" })`를 받아 AI가 직접 점검하고, 허위·상충·키워드 누락을 고친다.
8. 완성본을 `add_resume`으로 저장한다(`kind=career_description`, `source=ai`, title은 사용 목적이 드러나게 작성, tags에 직무/용도 키워드 포함). 대표 문서로 삼아도 되는지 확인된 경우에만 `is_primary=true`를 쓴다.
9. 저장 결과를 사용자의 언어로 알리고, 문서 탭에서 확인할 수 있음을 안내한다. 특정 공고용으로 작성했다면 이어서 자기소개서나 면접 준비를 제안한다.

**도구 호출 순서 요약:**

```
get_onboarding_status
  → get_profile / get_resumes / get_experiences / get_projects / get_skills
  → read_document | read_inbox (사용자가 파일을 준 경우)
  → get_workflow_guide({ workflow_id: "write_career_description" })
  → get_playbook({ domain: "resume" }) / get_playbook({ domain: "ats" })
  → (목적·대상 직무·분량 확인)
  → (경력기술서 작성: 요약·역량·경력/프로젝트·성과)
  → get_verifier({ id: "truthfulness" }) / get_verifier({ id: "consistency" }) / get_verifier({ id: "ats-compat" })
  → add_resume(kind=career_description, source=ai)
```

---

## 5. manage_application_status — 지원 상태 관리
...
**상태 전이 참고:** `document_passed` 이상에서는 면접 준비를 다음 행동으로 우선 제안한다. 8단계 상태값은 코드(`update_application_status`의 `inputSchema`)에 정의되어 있다.
...
## 6. prepare_interview — 면접 준비

공고 기준 예상 질문·STAR 답변·1분 자기소개를 준비해 저장하는 흐름.

**트리거:** 사용자가 "면접 준비 도와줘"라고 요청하거나, 지원 상태가 서류 합격(`document_passed`) 또는 면접 진행(`interview`)으로 바뀌었을 때.
...
5. 준비 내용을 사용자의 언어로 쉽게 요약해 전달하고, 모의 면접/추가 질문 연습이나 상태 업데이트(`update_application_status`로 `interview`/`final_passed`)를 제안한다.
...
## 7. build_personal_brand — 퍼스널 브랜드 (LinkedIn·포트폴리오)

LinkedIn 프로필과 포트폴리오 같은 자기표현 자산을 직무에 맞춰 작성·최적화하는 흐름. 이력서 임포트(`onboarding`)와 단계 의미가 달라(개인 브랜드 자산 *생성*) 별도 워크플로우로 둔다.

**트리거:** 사용자가 "링크드인 프로필 봐줘 / 포트폴리오 정리해줘 / 퍼스널 브랜딩 도와줘"라고 요청할 때.

**단계:**

1. `get_application_context`(가능하면 대상 `job_id` 포함)로 프로필·경력·프로젝트·스킬·대표 이력서·글쓰기 선호를 가져온다. 타깃 공고가 있으면 그 키워드를 사전 입력으로 쓴다.
2. `get_workflow_guide({ workflow_id: "build_personal_brand" })`로 전문가 절차를 받고, 작성 직전 `get_playbook({ domain: "linkedin-profile" })`·`get_playbook({ domain: "portfolio" })`로 헤드라인·About·3막(problem→approach→impact) 규범을 적용한다.
3. 무엇을 만들지 확인한다(LinkedIn/포트폴리오/둘 다, 타깃 직무, 강조 포인트). 저장된 실제 경험·성과만 쓰고 없는 수치·고유명사는 `[확인 필요]`로 표시해 묻는다.
4. 저장 직전 `get_verifier({ id: "human-voice" })`·`truthfulness`·`consistency`로 직접 점검한다(AI 티·환각·사실 보존).
5. 포트폴리오 본문은 `add_resume`(`kind=portfolio`)로 저장한다. LinkedIn 초안은 사용자가 복사해 쓰도록 제시한다(전용 저장·검증 도구는 Phase B).
...
onboarding
  ...
  → analyze_job ──(자소서 제안 동의)──→ write_cover_letter
       │
       └─→ manage_application_status ──(document_passed 이상이면 우선 제안)──→ prepare_interview
```

- `write_career_description`은 공고와 무관한 마스터본으로도 시작할 수 있고, 특정 공고용으로 작성한 뒤 `write_cover_letter`로 이어질 수도 있다.
...
- `build_personal_brand`(LinkedIn·포트폴리오)는 핵심 6 흐름과 독립적인 선택 작업으로, 사용자가 자기표현 자산을 요청할 때 진입한다.
