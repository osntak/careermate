...
| `export_resume` | 저장된 이력서·경력기술서 등 문서를 Markdown 또는 인쇄용 HTML 파일로 `exports/`에 저장 | `document_id`, `format?`(`md`(기본)/`html`) | `{ path, filename, content }` | 공고 맞춤 이력서를 `add_resume`로 저장한 뒤 "이력서 파일로 받고 싶어"라고 할 때 | ✏️ |
| `export_profile` | 프로필·경력·프로젝트·보유 기술을 한 장의 이력서 형식으로 모아 Markdown 또는 인쇄용 HTML 파일로 `exports/`에 저장 | `format?`(`md`(기본)/`html`) | `{ path, filename, content }` | 사용자가 "내 프로필을 이력서로 뽑아줘"라고 할 때 | ✏️ |
