...
이 문서는 `packages/mcp-tools/src/tools.ts`에 정의된 커리어 워크플로 **도구**의 레퍼런스입니다. 도구의 입력 파라미터는 `packages/shared/src/schemas.ts`의 zod 스키마와 일치합니다.
...
| --- | --- |
| `read_document`, `read_inbox`, `get_onboarding_status`, `get_profile`, `get_resumes`, `get_experiences`, `get_projects`, `get_skills`, `get_cover_letters`, `get_job_posting`, `list_jobs`, `prescreen_job`, `search_jobs`, `compare_offers`, `get_offer`, `get_application_context`, `list_recent_activity`, `get_followups`, `get_pipeline_stats`, `get_workflow_guide`, `get_playbook`, `get_verifier`, `validate_cover_letter`, `get_writing_style_guide`, `check_for_update` | `open_inbox`, `save_profile`, `add_resume`, `delete_resume`, `add_experience`, `delete_experience`, `add_project`, `delete_project`, `add_skill`, `delete_skill`, `save_cover_letter_version`, `delete_cover_letter`, `set_verify_mode`, `save_job_posting`, `delete_job_posting`, `save_fit_analysis`, `update_application_status`, `save_interview_prep`, `save_offer`, `export_cover_letter`, `export_resume`, `export_profile`, `open_dashboard`, `open_application`, `update_careermate` |
...
- `pdf`·`docx`·`hwp`/`hwpx`·텍스트는 본문 텍스트를 추출하고, **텍스트 레이어가 없는 이미지·스캔본 PDF만** 추출되지 않아 파일 경로만(`unsupported`) 돌려주니 AI 클라이언트의 파일 읽기 기능으로 그 경로를 직접 열어 읽으세요.
...
| `delete_resume` | 저장된 문서(이력서·경력기술서·포트폴리오) 삭제 | `document_id`, `confirm`(=`DELETE`) | `{ deleted, id }` | 사용자가 특정 문서 삭제를 명확히 요청했을 때(먼저 get_resumes로 확인) | ✏️ |
...
| `delete_experience` | 경력 항목 1건 삭제 | `experience_id`, `confirm`(=`DELETE`) | `{ deleted, id }` | 잘못 추출/입력된 경력을 지울 때(먼저 get_experiences로 확인) | ✏️ |
...
| `delete_project` | 프로젝트 항목 1건 삭제 | `project_id`, `confirm`(=`DELETE`) | `{ deleted, id }` | 잘못 추출/입력된 프로젝트를 지울 때(먼저 get_projects로 확인) | ✏️ |
...
| `delete_skill` | 기술 항목 1건 삭제 | `skill_id`, `confirm`(=`DELETE`) | `{ deleted, id }` | 잘못 추출/입력된 기술을 지울 때(먼저 get_skills로 확인) | ✏️ |

- `delete_*`는 항목 1건만 지웁니다. **수정**은 별도 도구 없이 `add_*`로 같은 항목(같은 식별자)을 다시 저장하면 갱신됩니다(멱등).
...
| `save_cover_letter_version` | 자기소개서의 새 버전 저장 — 자소서 작성 워크플로우의 핵심 저장 단계 | `content`(필수), `cover_letter_id?`, `title?`, `job_id?`, `note?`, `source?`, `set_current?`, `force?`, `strict?` | `{ coverLetter, version }` | 새 공고용 자소서를 생성·갱신해 버전으로 남길 때 | ✏️ |
...
| 도구 | 목적 | 주요 입력 | 반환 | 언제 호출 | 읽기전용 |
| --- | --- | --- | --- | --- | --- |
| `search_jobs` | 키워드로 무인증 공개 잡보드(원티드·점핏)에서 공고를 *가져옴*(저장은 안 함) | `keyword`(필수), `sources?`, `limit?`(기본 10) | `{ query, results[](회사·직무·url·지역·경력·기술·마감), providers[] }` | 사용자가 "○○ 공고 찾아줘"라고 할 때. 결과를 보여주고 **고른 것만** save_job_posting | ✅ |
...
| `prescreen_job` | 공고 키워드 ↔ 사용자 보유 기술 겹침을 결정론으로 계산(값싼 사전 필터, LLM 없음) | `job_id` | `{ job_keywords, matched[], missing[], coverage, user_skill_count }` | 본격 적합도 분석 전 "깊게 볼 가치 있나" 빠르게 가늠할 때(여러 공고 우선순위) | ✅ |
...
받은 채용 제안(오퍼)을 공고에 연결해 저장·비교합니다(공고당 1건). 점수·최종평은 **연결된 AI가 저장 시 작성**하고(CareerMate엔 LLM 없음), 대시보드는 그걸 렌더링합니다. 정밀 비교는 `compare_offers` 데이터를 받아 AI가 `offer-evaluation`·`salary-negotiation` 관점으로 수행합니다.

| 도구 | 목적 | 주요 입력 | 반환 | 언제 호출 | 읽기전용 |
| --- | --- | --- | --- | --- | --- |
| `save_offer` | 오퍼 저장(공고당 1건, 갱신). 현금성은 만원 단위 숫자, AI가 `ai_score`·`verdict`(최종평) 함께 작성 | `job_id`(필수), `base_salary?`·`bonus_amount?`·`welfare_amount?`·`signing_amount?`(만원), `equity_note?`, `comp_note?`, `work_arrangement?`, `contract_type?`, `accept_deadline?`, `ai_score?`, `verdict?`, `notes?` | 저장된 오퍼(+`total_comp_annual_est` 결정론 합산) | 사용자가 오퍼를 받아 저장·평가할 때 | ✏️ |
| `compare_offers` | 저장된 모든 오퍼를 점수순으로 회사·직무·현금성 총액추정·최종평과 함께 반환 | 없음 | `OfferWithJob[]`(점수 desc) | "오퍼 비교해줘" 또는 여러 오퍼를 함께 판단할 때 | ✅ |
| `get_offer` | 특정 공고의 오퍼 1건 조회 | `job_id` | 오퍼 또는 `null` | 한 공고의 오퍼를 확인할 때 | ✅ |

- `total_comp_annual_est`(연 현금성 추정) = 기본급 + 성과급 + 복지(일시금 제외), CareerMate가 결정론으로 합산. 없으면 `null`.
- 회사 평판은 `save_job_posting`의 `reputation`에 저장해두면 오퍼 판단·면접에 함께 활용됩니다.
...
| 도구 | 목적 | 주요 입력 | 반환 | 언제 호출 | 읽기전용 |
| --- | --- | --- | --- | --- | --- |
| `save_interview_prep` | 면접 *준비*(예상 질문·STAR·자기소개)와 면접 *후 디브리프*를 한 공고에 저장(같은 공고면 갱신) | `job_id`(필수), `questions?`, `star_guides?`, `self_introduction?`, `notes?`, `debrief?` | 저장된 `InterviewPrepRecord`(질문 수 포함) | 사용자가 공고 기준 면접 준비를 요청했을 때, 또는 면접을 본 뒤 디브리프를 남길 때 | ✏️ |

입력 구조(`InterviewPrepInputSchema`):

- `questions[]`: `{ question, category?('technical'|'behavioral'), intent?(질문 의도), followups?(예상 꼬리 질문), answer_outline?(답변 가이드/핵심 포인트) }`
- `star_guides[]`: `{ question, situation?, task?, action?, result? }`
- `self_introduction`: 1분 자기소개 초안
- `notes`: 면접 후기/메모
- `debrief`(면접 후): `{ asked_questions?(실제로 받은 질문), went_well?, to_improve?, next_round_focus?(다음 라운드 대비), sentiment?('positive'|'neutral'|'negative'), interviewed_at? }` — 면접 전 준비와 한 엔티티에 함께 쌓이며, 부분 저장 시 기존 디브리프 필드와 **병합**(덮어쓰지 않음). 다음 라운드 준비·일관성 점검에 활용.
...
| 도구 | 목적 | 주요 입력 | 반환 | 언제 호출 | 읽기전용 |
| --- | --- | --- | --- | --- | --- |
| `export_cover_letter` | 자기소개서를 Word(`.docx`)·Markdown·인쇄용 HTML 파일로 데이터 폴더 `exports/`에 저장 | `cover_letter_id`, `format?`(`md`(기본)/`html`/`docx`) | `{ path, filename, content }`(docx는 `content`가 안내문, 실제 파일은 `path`) | 사용자가 "자소서 파일로 받고 싶어"라고 할 때(ATS 제출이면 `docx`) | ✏️ |
| `export_resume` | 저장된 이력서·경력기술서 등 문서를 Word(`.docx`)·Markdown·인쇄용 HTML 파일로 `exports/`에 저장 | `document_id`, `format?`(`md`(기본)/`html`/`docx`) | `{ path, filename, content }`(docx는 `content`가 안내문) | 공고 맞춤 이력서를 `add_resume`로 저장한 뒤 "이력서 파일로 받고 싶어"라고 할 때(ATS 제출이면 `docx`) | ✏️ |
| `export_profile` | 프로필·경력·프로젝트·보유 기술을 한 장의 이력서 형식으로 모아 Word(`.docx`)·Markdown·인쇄용 HTML·JSON Resume 파일로 `exports/`에 저장 | `format?`(`md`(기본)/`html`/`docx`/`json`) | `{ path, filename, content }`(docx는 `content`가 안내문) | 사용자가 "내 프로필을 이력서로 뽑아줘"라고 할 때, 또는 다른 이력서 도구·테마와 연동할 표준 데이터가 필요할 때(`json`) | ✏️ |

- Word(`.docx`)는 ATS 제출에 알맞은 단일 컬럼 표준 레이아웃입니다(회사 지정 양식 채우기는 아직 지원하지 않습니다).
- HTML은 인쇄용으로, 브라우저에서 PDF로 저장할 수 있습니다.
- `export_profile`의 `json`은 [JSON Resume](https://jsonresume.org) v1.0.0 표준 형식입니다 — 100+ 테마(HTML/PDF 렌더)·다른 이력서/ATS 도구와 상호운용됩니다. 빈 필드는 생략하고 저장된 날짜는 그대로 내보냅니다.
- 대시보드의 Documents·Profile 페이지에서 직접 다운로드할 수도 있습니다.
...
| `get_followups` | 지금 챙길 항목 한 번에 — 무응답 지원(지원 후 무응답·서류합격 후 일정대기·면접 후 결과대기)·임박/지난 마감(지원 전 단계)·면접 준비 누락 공고 | 없음 | `{ today, followups[], deadlines[], interview_todo[] }` | "내 지원 현황 어때?/뭐 챙겨야 해?" 또는 세션 이어가며 놓친 것 점검할 때 | ✅ |
| `get_pipeline_stats` | 지원 파이프라인 분석 — 현재 상태 분포·단계별 도달 깔때기(지원→서류→면접→최종, 상태 변경 이력 복원)·통과율·결과 요약 | 없음 | `{ total, by_status[], funnel, rates, outcomes, note }` | "내 합격률/지원 통계 어때?", "어느 단계에서 떨어져?"라고 물을 때(비율은 설명용, 표본 작으면 추세만) | ✅ |
...
`workflow_id` 값: `onboarding`, `analyze_job`, `write_cover_letter`, `write_career_description`, `manage_application_status`, `prepare_interview`, `build_personal_brand`. 생략하면 전체 목록을 돌려줍니다. `analyze_job`/`write_cover_letter`/`write_career_description`/`prepare_interview`/`onboarding`은 해당 작업의 EOP 본문과 `get_playbook`·`get_verifier` 호출 순서(`career_route`)를 함께 돌려줍니다.
...
- `get_verifier` id(7): `truthfulness`, `consistency`, `recency-staleness`, `responsiveness-on-target`, `ats-compat`, `human-voice`, `ai-detection-risk`(한국 AI-자소서 탐지 시장 전용 — 진정성·면접 방어가능성·자기복제 점검).
...
| 도구 | 목적 | 주요 입력 | 반환 | 언제 호출 | 읽기전용 |
| --- | --- | --- | --- | --- | --- |
| `get_writing_style_guide` | "AI 티 안 나는" 한국어 글쓰기 규칙 반환 | `doc_type?`(career_description·resume·portfolio면 구조 보존 보정 추가) | `{ guide }` (작성 규칙 + 저장 전 자가 점검) | 자기소개서·자기 PR·지원 메일·경력기술서 등 사람이 쓴 듯한 글을 작성하기 직전 | ✅ |
...
| `today` | 서버의 오늘 날짜(`YYYY-MM-DD`). LLM이 현재일을 신뢰성 있게 모르므로 주입(연차·신선도 판단 기준; MCP 응답 한정) |
| `tenure` | 재직기간을 서버가 결정적으로 계산한 배열 — 각 `{ company, is_current, start_date, as_of, months, years, data_age_months }`. **재직중=등록일 `updated_at` 기준 연차, 종료=`end_date` 기준.** 연차는 직접 산술하지 말고 이 값을 사용(`data_age_months` 크면 현재 재직 확인 필요). MCP 응답 한정 |
