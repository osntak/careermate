...
| `export_profile` | 프로필·경력·프로젝트·보유 기술을 한 장의 이력서 형식으로 모아 Word(`.docx`)·Markdown·인쇄용 HTML 파일로 `exports/`에 저장 | `format?`(`md`(기본)/`html`/`docx`) | `{ path, filename, content }`(docx는 `content`가 안내문) | 사용자가 "내 프로필을 이력서로 뽑아줘"라고 할 때 | ✏️ |
