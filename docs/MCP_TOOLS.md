...
이 문서는 `packages/mcp-tools/src/tools.ts`에 정의된 커리어 워크플로 **35개 도구**의 레퍼런스입니다. 도구의 입력 파라미터는 `packages/shared/src/schemas.ts`의 zod 스키마와 일치합니다.
