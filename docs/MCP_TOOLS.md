...
| `get_anchor` | 구조화 조회 데이터(앵커). 수치·등급·분류·동의어·동사 레벨을 발명하지 말고 조회 | `asset`(8개 enum) | `{ asset, content }` | 보통 `get_playbook`에 자동 첨부됨 — 단건만 다시 볼 때 호출 | ✅ |
...
- `get_anchor` asset(8): `kr-question-directives`, `charcount-modes`(자소서) · `seniority-verb-register`, `transferable-skill-adjacency`, `knockout-criteria-taxonomy`, `fit-self-check-rubric`(적합도) · `competency-enum`(면접) · `ncs-official`(경력기술서). 해당 도메인 `get_playbook` 출력에 **자동 첨부**되며(추가 왕복 0), 정본은 `docs/career-os/knowledge/anchors/`. advisory 데이터(의미 필드는 AI 판단). 근거: [`career-os/validation/research/2026-06_knowledge-enrichment.md`](career-os/validation/research/2026-06_knowledge-enrichment.md).
