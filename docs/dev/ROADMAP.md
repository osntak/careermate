...
이 문서는 현재 버전(`0.0.1`)의 MVP 완료 범위와, 지금은 제외하되 구조는 열어둔 향후 과제를 정리한다.
...
- [x] **커리어 워크플로 MCP 도구 (33개)** — 전체 최신 목록은 [`MCP_TOOLS.md`](../MCP_TOOLS.md):
  - 온보딩/프로필: `get_onboarding_status`, `save_profile`, `get_profile`
  - 문서: `add_resume`, `get_resumes`, `get_cover_letters`
  - 경력·프로젝트·스킬: `add_experience`, `get_experiences`, `add_project`, `get_projects`, `add_skill`, `get_skills`
  - 공고: `save_job_posting`, `get_job_posting`, `list_jobs`
