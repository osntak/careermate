...
- **MCP stdio 서버** (`apps/mcp`) — AI 클라이언트(Claude Desktop, ChatGPT, Cursor 등)가 `npm run mcp` 형태로 기동. stdio 위에서 MCP 프로토콜로 통신하며, 35개의 CareerMate 도구를 노출한다.
...
| `packages/mcp-tools` | `@careermate/mcp-tools` | 35개 MCP 도구 정의(`TOOLS`)와 핸들러, `toCallToolResult`. 도구 핸들러는 core를 호출. | core, shared, parsers/prompts/workflows |
...
- MCP 도구 목록(35개): [MCP_TOOLS.md](../MCP_TOOLS.md)
