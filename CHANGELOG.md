...
### 변경 (Changed)
- 자기소개서 작성 지침 강화: 기존 자소서를 회사명·일부 문구만 바꿔 재사용하는 "자기복제"를 막고(한국 표절·AI 탐지 대응), 사실·수치·보이스는 공고 간 일관되게 두되 직무 적합 앵글·도입·문장은 공고마다 새로 쓰도록 안내.
- 여러 문서를 한 번에 만들어도 각 문서를 각자의 저장 도구로 따로 저장하도록 가드레일 추가(경력기술서·자기소개서 혼입 방지).
- 자기소개서 자가검증에 문항 응답·글자수 점검을, 지원 상태 관리·면접 준비에 정보 신선도 점검을 추가해 한국 문항형 자소서와 시점 민감 정보의 품질을 높임.
- 채용공고를 링크(URL)로 주면 AI가 그 페이지 본문을 직접 읽어 저장하도록 안내(웹을 못 쓰는 환경이면 본문 붙여넣기 요청).

### 수정 (Fixed)
- 인입 폴더 일괄 읽기(`read_inbox`)가 PDF 본문도 추출함을 문서에 정확히 반영(추출하지 않는다는 설명 오류 수정).
- 워크플로우 안내 문서의 누락 항목(개인 브랜드 워크플로우 등) 보완.
