...
export const ONBOARDING_PROMPT = `이제 CareerMate에 연결되었습니다. 사용자의 커리어 데이터 첫 셋업(온보딩)을 도와주세요. 사용자가 쓰는 언어로 안내하세요 — 이 안내문이 한국어라도 사용자가 다른 언어로 말하면 그 언어로 답합니다.
...
- 사용자에게 "지금까지 등록된 정보"와 "앞으로 채우면 좋은 정보"를 사용자의 언어로 쉽고 짧게 안내하세요. (이미 채워진 항목은 다시 묻지 마세요.)
...
진행 내내 기술 용어(MCP, job_id 등)는 빼고, 지금 무엇을 하고 있는지 사용자의 언어로 쉽게 알려주세요.`;
